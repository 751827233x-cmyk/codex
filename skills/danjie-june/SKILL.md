...
2. If producing a full storyboard execution draft, load `references/critical-error-control-system.md`, `references/full-guidelines.md`, `references/axis-continuity.md`, `references/high-density-storyboard.md`, `references/industrial-control-system.md`, `references/staging-entrance-system.md`, `references/cinematic-shot-language.md`, `references/camera-cutting-spatial-methodology.md`, and `references/action-function-causality.md`; follow all nine as the authoritative specification.
3. Before splitting, perform a compact control pass: identify source mode, name mode, source facts, core conflict, true plot boundaries, active character count, S0/S1 severe-risk beats, load level, dominant timing driver, opening/entrance/exit staging needs, action-function and cause-effect chain, spatial state, master spatial relationship, shot-language strategy, shot-capacity risks, prompt-semantics risks, ledger-amplification risks, sound plan, and opening/escalation/aftermath visual anchors.
...
5. Establish the scene's aspect ratio, action line, camera safety side, screen-left/screen-right assignments, frame-safe zones, and screen lanes before writing shots. If the user specifies horizontal, vertical, square, or cinematic widescreen, adapt all capacity rules to that format; if unspecified, state the working assumption in the ledger.
6. Track each character's position, posture, facing direction, props, visible condition, emotion, eye-line, screen direction, and frame lane from segment to segment.
...
- Run the S0/S1 stop gate from `critical-error-control-system.md` before drafting and again before delivery. If source truth, plot causality, action function, spatial axis, shot capacity, time/process, aspect ratio, prompt semantics, ledger consistency, or character state is wrong, rewrite before output.
- Before writing action, interruption, attack, warning, exit, pursuit, spell, weapon, obstacle, or restraint beats, classify the action function using `action-function-causality.md`. Do not turn a character's reaction, fear, hesitation, or obedience into a false physical state such as a blocked path, sealed exit, hit body, or restraint unless the source clearly requires it.
...
- A newly arrived character cannot simply appear in final position. State entry direction, threshold or reveal cue, final frame lane, who reacts first, and how the existing axis updates without flipping screen direction.
...
- Lock screen direction for the current aspect ratio. State who stays screen-left/screen-center/screen-right, who looks screen-left/screen-right, and which frame lane each subject occupies. Use vertical lane percentages only for vertical formats; use horizontal/depth zones for horizontal formats.
...
- In close shots and over-shoulder shots, preserve orientation by naming the visible shoulder, off-screen eye-line, screen direction, and frame lane.
...
- In AI video prompt lines, include only visually useful continuity words such as `入口侧同轴`, `同侧过肩`, `A左/B中/C右`, `前景/中景/后景`. Avoid verbose labels like `轴线：新场景建轴 + 屏幕方向：... + 画幅构图：...` in every shot.
...
- Match the visual pleasure to the source genre or the user's requested tone. For female-oriented realistic爽剧 only when requested or clearly implied, emphasize emotional contrast, clean but high-pressure lighting, restrained romantic rim light, shallow depth of field, and reaction-shot atmosphere without adding decorative text or sticker effects.
...
分秒运镜及切镜视频提示词
2秒 景别 / 运镜方式（速度） / 构图方式 + 简短空间锁定词（如入口侧同轴、A左/B中/C右、同侧过肩、前中后景）；镜头从某个动作或空间锚点开始，按明确路径移动或保持有意图的弱运动，落到情绪、权力、揭示、反应、对比或余韵重点；构图、距离、焦点、遮挡、高低或留白必须直接呈现在画面句里，不写解释性理由；当前画幅和景别容量装不下的对象写成画外方向、焦外存在或另切关系镜，不能强行同框；角色开口前有呼吸、停顿、眼神或手部动作，说“原台词/原OS/原VO”，说完后有余韵反应；加入环境声、动作声或物理反馈；[光效]
...
Before finalizing, check the draft against `references/critical-error-control-system.md`, `references/full-guidelines.md`, `references/axis-continuity.md`, `references/high-density-storyboard.md`, `references/industrial-control-system.md`, `references/staging-entrance-system.md`, `references/cinematic-shot-language.md`, `references/camera-cutting-spatial-methodology.md`, and `references/action-function-causality.md`, especially:

- no S0/S1 severe error remains: source truth, plot causality, action function, spatial/axis logic, shot capacity, time/process, aspect ratio, prompt semantics, ledger consistency, and character state all pass
- any new rule or correction is generalized as `error family -> root cause -> decision method -> output constraint -> self-check`, not tied to one plot, weapon, genre, location, character, or frame format
...
- every shot passes the single-shot capacity gate: shot size, current aspect ratio, distance, depth, and visibility state are compatible
...
- current-aspect lane stability and safe-area framing
